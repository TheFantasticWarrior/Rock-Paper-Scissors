...
to pickle the bots scope dictionary."""
    ...
    def set_genes(self,n):
        if type(n)==int:
            self.genes=[n, 171.18, -1.55, -0.33, -0.79, 0.43, 0.73, 11, -2, 33, -41, -326, 4572.77, 56.51, -0.0, 0.89, 0.5, -0.16, -0.01, 0.08, -0.08]
            for i in range(1,21):
                self.genes[i]=round((random.random()-0.5+self.genes[i])*(random.random()*3-0.5),2) 
            for i in range(7,12):
                self.genes[i]=int(self.genes[i])
            if self.genes[20]<0:
                self.genes[20]=-self.genes[20]
        else:
            self.genes=n
        self.scope["genes"]=self.genes
...
desc is an optional output string."""
    ...
    for i in range(len(names)):
        if bot_obj(names[i]) in bots:
            names[i]+=str(i)
        bots.append(bot_obj(names[i]))
    print(f"{len(bots)} bots loaded")
    return bots


def match_maker(bots, bots2=None, matches=10, rounds=1000):
...
def report_results(bots1,bots2, results):
    ...
    win_ratio = [(x,float(rounds_won[x])/rounds_played[x]) for x in botnames]
    ...
    top=[bot for bot in botnames if bot not in [bot1.name for bot1 in bots1]][:round(len(botnames)/1)]
    f=open("genes.txt","w")
    for bot in top:
        if scores[bot]<-5:
            break
        b=bots2[bots2.index(bot)]
        f.writelines(str([float(scores[bot])/ matches_played[bot],b.genes])+"\n")
    f.close()
...
def main(prog_args):
    ...
    if len(args) > 1:
        if "*" in args[1]:
            ...
            name,count=args[1].split("+")
            count=int(count.split(".")[0])
            bots2_files = [f"{name}{i}.py" for i in range(count)]
            g=1
        elif "=" in args[1]:
            name,count=args[1].split("=")
            count=int(count.split(".")[0])
    ...
    for i in range(100):
        ...
        genesmodify(count,"genes.txt","genes2.txt")
